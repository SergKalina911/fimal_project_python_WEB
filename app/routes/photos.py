...
# from app.routes.users import user_to_schema   # 🔹 імпортуємо хелпер
...
#     # 🔹 серіалізуємо owner через user_to_schema, щоб був photo_count
...
#     # трансформуємо фото через Cloudinary
...
#         await db.commit()
#     else:
#         # 🔹 якщо нові теги не передані — копіюємо старі
#         for tag in photo.tags:
#             await db.execute(photo_tags.insert().values(photo_id=new_photo.id, tag_id=tag.id))
#         await db.commit()
    
#     # Створюємо QR-код для нового фото
...
# # --- додавання тегів ---
# @router.post("/{photo_id}/tags", response_model=PhotoRead)
# async def add_tags(
#     photo_id: int,
#     tag_names: list[str],
#     db: AsyncSession = Depends(get_db),
...
#     )
#     photos = result.unique().scalars().all()   # ✅ тут додано .unique()
...
#     # 🔹 очищаємо кеш ORM (синхронний виклик, без await)
#     db.expire_all()
